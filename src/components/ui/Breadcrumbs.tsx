...
const Breadcrumbs: React.FC = () => {
  ...
  useEffect(() => {
    const currentMenuItem = menuItems.find(
      (item: MenuItem) => item.link !== undefined && item.link === locationName
    );

    const currentChild = menuItems.find((item: MenuItem) =>
    ...
    if (currentMenuItem) {
      ...
      const child = currentChild.child?.find(
        (child) => child.childlink === locationName
      );
      setIsHide((child as any)?.isHide || false);
